...
class SchedulerMixin:
    ...
    def _enqueue_requests(self, requests_iterable: Iterable[Request], requests_queue: multiprocessing.Queue):
        """Add `Request` iterable to `requests_queue`."""
        self._enqueue_items(requests_iterable, requests_queue)

    def _enqueue_responses(self, responses_iterable: Iterable[Response], responses_queue: multiprocessing.Queue):
        """Add `Response` iterable to `responses_queue`."""
        self._enqueue_items(responses_iterable, responses_queue)

    @staticmethod
    def run_callables_in_pool_executor(callables_and_args: Iterable[tuple[Callable, Any]], max_workers: Optional[int] = None):
        """Run callables in a ProcessPoolExecutor."""
        max_workers = max_workers or len(callables_and_args)
        with ProcessPoolExecutor(max_workers=max_workers) as pool:
            futures = [pool.submit(*callable_and_args) for callable_and_args in callables_and_args]
            for future in futures:
                future.result()  # Ensure all futures complete

class RequestWorker(SchedulerMixin):
    """Abstraction of a worker responsible for consuming Requests from the requests_queue and appending Responses to response_queue."""
    ...
    def __call__(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue):
    ...
    def get_client_by_name(self, client_name: str | None = None) -> Client:
        ...
        if client_name is not None:
            logger.warning(f"No client with name {client_name} found. Falling back to main client.")
        return self.get_main_client()

    async def __process_requests_async(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue):
    ...
    def __process_requests(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue):
        """Sync wrapper around `__process_requests_async`."""
        return async_to_sync(self.__process_requests_async, requests_queue, responses_queue)

class ResponseWorker(SchedulerMixin):
    def __call__(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue, data_queue: multiprocessing.Queue):
        """Main entry point. This method is called by the DataService."""
        self.__process_responses(requests_queue, responses_queue, data_queue)

    def _process_response(self, response: Response, requests_queue: multiprocessing.Queue, data_queue: multiprocessing.Queue):
        ...
        if isinstance(callback_result, Generator):
            for item in callback_result:
                if isinstance(item, Request):
                    ...
                    raise ValueError(f"Unknown type: {type(item)}. Expected Data or Request.")
        else:
            if isinstance(callback_result, Request):
                ...
                raise ValueError(f"Unknown type: {type(callback_result)}. Expected dict or Request.")

    def __process_responses(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue, data_queue: multiprocessing.Queue):
...
class DataService(SchedulerMixin):
    ...
    def __run_processes(self, requests_queue: multiprocessing.Queue, responses_queue: multiprocessing.Queue, data_queue: multiprocessing.Queue):
