...
One common issue when using long chains in ``BeautifulSoup`` is that the code will raise an exception if there's a method call or property access on a missing attribute.
E.g.
...
Using try-except blocks or if conditions for each attribute is not very elegant.

For this specific purpose, ``BaseDataItem`` implements a global model pre-validator that will invoke callable values and store any exception in the ``errors`` dictionary.
...
We also don't want to hammer the server with too many requests, so we add a delay between requests using the ``ServiceConfig`` object.

``DataService`` doesn't come with logging on out of the box, however, it provides a utility function to set up the logging for you.
